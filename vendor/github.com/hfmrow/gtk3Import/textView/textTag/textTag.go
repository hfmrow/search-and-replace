...
func TagCreateIfNotExists(textBuffer *gtk.TextBuffer, tagName string, props map[string]interface{}) (tag *gtk.TextTag) {
	var ok bool
	if tag, ok = TagLookupIfExists(textBuffer, tagName); ok {
		for name, value := range props {
			tag.SetProperty(name, value)
		}
	} else {
		tag = textBuffer.CreateTag(tagName, props) // add tag & properties
	}
	return
}

// RemoveTag: from entire buffer if exists.
func TagRemoveIfExists(textBuffer *gtk.TextBuffer, tagName string) {
	if tag, ok := TagLookupIfExists(textBuffer, tagName); ok {
		textBuffer.RemoveTag(tag,
...
// LookupExistingTag:
func TagLookupIfExists(textBuffer *gtk.TextBuffer, tagName string) (tag *gtk.TextTag, ok bool) {
	if tagTable, err := textBuffer.GetTagTable(); err == nil { // Get table
